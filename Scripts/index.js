...
const createPostButton2 = document.querySelector("#createPostButton2");
...
createPostButton2.addEventListener('click', openCreatePost);
...
const createStoryButton = document.querySelectorAll('#createStoryButton');
...
createStoryButton.forEach(item => {
    item.addEventListener('click', openCreateStory);
});
...
closeCommentByHome.addEventListener('click', () => {
    commentsView.forEach(commentsViewElement => {
        if (commentsViewElement.style.display !== 'none') {
            commentInput.forEach(commentsInputElement => {
                if (commentsInputElement.value !== '') {
                    commentsInputElement.value = '';
                }
...
const closeEditComment = () => {
    editCommentButton.forEach(item => {
        item.classList.remove('active');
    })
}
...
document.addEventListener("click", function(event) {
  ...
  var isClickInside = editComment.contains(event.target);

  if (!isClickInside) {
    editCommentButton.forEach(function(button) {
      if (button === event.target || button.contains(event.target)) {
        return;
      }
      button.classList.remove("active");
...
postIcon.addEventListener('touchstart', () => {
    postIcon.style.transition = "all 0ms ease";
    postIcon.style.backgroundColor = "hsl(252, 30%, 95%)";
});



postIcon.addEventListener('touchend', () => {
    postIcon.style.backgroundColor = "initial";
    postIcon.style.transition = "all 300ms ease";
});
...
function toggleCards() {
    var christmasCardContainer = document.getElementById("christmas-card-container");
